...
const { User, Role, Teacher } = require('../lessonPlanModels');
...
const login = async (req, res) => {
    try {
        ...
        let user = await User.findOne({
            where: { email },
            include: [Role, { model: require('../lessonPlanModels').School, required: false }]
        });
        
        fs.writeFileSync('/tmp/login_debug.log', `Email: ${email}\nPassword: ${password}\nUser found: ${!!user}\n`);
        ...
        fs.appendFileSync('/tmp/login_debug.log', `Hash match: ${isMatch}\n`);
        if (!isMatch) {
