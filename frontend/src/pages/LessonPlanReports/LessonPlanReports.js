...
const LessonPlanReports = () => {
    ...
    const fetchLectures = async () => {
        try {
            const res = await api.get('/lesson-plan-lectures');
    ...
    const fetchPdf = async (lectureId) => {
        ...
        setPdfBlobUrl(null);
        try {
            const response = await api.get(`/lesson-plan-analysis/${lectureId}/download`, {
                responseType: 'blob',
            });
    ...
    const handleApproveReport = async (lectureId) => {
        try {
            await api.put(`/lesson-plan-lectures/${lectureId}/approve`);
...
export default LessonPlanReports;
